...
class DeliveryNote(SellingController):
    ...
    def on_submit(self):
        self.update_sales_order_with_serial_numbers123()
...
# def update_serial_no_sales_order(self,method):
#     for item in self.items:
#         item_name = item.item_name
#         qty = item.qty
#         serial_and_batch_bundle = item.serial_and_batch_bundle
...
#         if against_sales_order:
#             self.update_sales_order_with_serial_numbers(against_sales_order, serial_no, item.item_code)

# @frappe.whitelist()
# def update_sales_order_with_serial_numbers(self, sales_order_name, serial_no, item_code):
#     try:
#         sales_order = frappe.get_doc("Sales Order", sales_order_name)

#         for so_item in sales_order.items:
#             if so_item.item_code == item_code:
#                 so_item.serial_no = serial_no
